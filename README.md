...
- 线上 Demo 已可访问：`https://shopclip.site`，推荐评审入口为 `https://shopclip.site/#project`，API 健康检查为 `https://shopclip.site/health`。
...
| 部署       | 线上服务器 + Render Blueprint 备选路径       |
...
最近一次完整验证记录在 `projects/shopclip-ai/evidence/` 和 `projects/shopclip-ai/02-development-plan.md` 的最新 addendum 中。
...
- API health：`https://shopclip.site/health`

仓库同时保留 Render Blueprint 作为可复现部署方案。
...
当前生产域名为 `shopclip.site`。如需在 Render 上复刻环境，仍需在账号侧创建 Blueprint 并填写最终环境变量。
...
- 作品提交文档：`projects/shopclip-ai/submission-document.md`
