...
export const SmartEditPanel = ({
  ...
  useEffect(() => {
    ...
    setSelectedSegmentIds((current) =>
      retainSmartEditSelectionIds({
        ...
        selectedIds: current,
        validIds: selectSmartEditSegmentIds(sortedSegments),
      }),
    );
  ...
  const openTimelineContextMenu = (
    ...
    if (params.clipId) {
      selectTrackClipIds([params.clipId]);
    }
    if (params.segmentId) {
      selectSegmentIds([params.segmentId], params.segmentId);
    }
  ...
  const timedTimelineSegments = useMemo(() => {
    return buildTimedSmartEditSegments(sortedSegments);
  }, [sortedSegments]);
  ...
  const trackBoxSelectTrackIdSet = useMemo(
    () => buildSmartEditTrackBoxSelectTrackIdSet(trackBoxSelectDrag),
    [trackBoxSelectDrag],
  );
  ...
  const selectTrackClipIds = (ids: string[], activeId = ids.at(-1)) => {
  ...
  const addVoiceElementAtPlayhead = () => {
    ...
    if (addedElement) {
      selectTrackClipIds([addedElement.id]);
      setSelectedSegmentIds([]);
    }
  };
  ...
  const selectTimelineSegment = (
    ...
    if (isRange && selectedSegment) {
      const rangeIds = selectSmartEditSelectionRangeIdsOrUndefined({
        orderedIds: selectSmartEditSegmentIds(sortedSegments),
    ...
    selectSegmentIds([segmentId], segmentId);
    clearSelectedTrackClips();
  };
  ...
  const selectTrackClip = (
    ...
    const trackClipIds = selectSmartEditTrackClipIds(trackSegments);
    ...
    selectTrackClipIds([trackClip.id]);
    if (trackClip.segmentId) {
      selectSegmentIds([trackClip.segmentId], trackClip.segmentId);
    } else {
      clearSelectedSegments();
    }
  };
  ...
  const selectAllSegments = () => {
    ...
    selectSegmentIds(selectSmartEditSegmentIds(sortedSegments), sortedSegments[0]?.id);
    clearSelectedTrackClips();
  };
  ...
  const removeSegments = (segmentIds: string[]) => {
    ...
    selectSegmentIds(nextSelectedId ? [nextSelectedId] : [], nextSelectedId);
  };
  ...
  const moveSelectedTrackClips = (deltaSeconds: number) => {
    ...
    const movableClipIds = selectMovableSmartEditTimelineMaterialIdsOrUndefined(
      selectedBatchTrackClips,
      isTimelineTrackLocked,
    );
    if (!movableClipIds) {
  ...
  const moveSelectedSegmentEarlier = () => {
    if (!plan || !selectedSegment) {
      return;
    }
  ...
  const moveSelectedSegmentLater = () => {
    if (!plan || !selectedSegment) {
      return;
    }
  ...
  const finishTrackBoxSelectDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const selectedIds = selectSmartEditTimelineElementIdsInBox(plan, {
      endSecond,
      startSecond,
      trackIds: Array.from(buildSmartEditTrackBoxSelectTrackIdSet(trackBoxSelectDrag)),
    });
    if (selectedIds.length === 0) {
      clearSelectedTrackClips();
      return;
  ...
  const removeSelectedTrackClip = () => {
    ...
    clearSelectedTrackClips();
    if (selectedTrackClip.segmentId) {
      selectSegmentIds([selectedTrackClip.segmentId], selectedTrackClip.segmentId);
    } else {
      clearSelectedSegments();
    }
  };
  ...
  const duplicateSelectedSegment = () => {
    ...
    if (duplicateSegment) {
      selectSegmentIds([duplicateSegment.id], duplicateSegment.id);
    }
  };
  ...
  const alignSelectedTimelineMaterialsToPlayhead = (edge: "start" | "end") => {
    ...
    const selectedTimelineMaterialIds = selectedEditableTimelineMaterialIdsOrUndefined();
  ...
  const duplicateSelectedTimelineMaterials = () => {
    ...
    if (duplicatedIds.length > 0) {
      selectTrackClipIds(duplicatedIds, duplicatedIds[0]);
    }
  };
  ...
  const duplicateSelectedSegments = () => {
    ...
    if (duplicateIds.length > 0) {
      selectSegmentIds(duplicateIds, duplicateIds[0]);
    }
  };
  ...
  const pasteSelectedSegmentsAtPlayhead = () => {
    ...
    if (pastedIds.length > 0) {
      selectSegmentIds(pastedIds, pastedIds[0]);
    }
  };
