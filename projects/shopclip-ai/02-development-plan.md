...
## 2026-06-09 Memory Store Render Scene Utility Addendum
...
- Moved ownership for:
  - render-task status to project-status mapping.
  - trace event materialization.
  - replacing a scene across project scenes and script scenes.
  - scene reorder validation/materialization.
  - syncing scripts to a new scene list.
  - removing a scene and preserving script scene consistency.
...
- `apps/api/src/modules/projects/memoryProjectStoreUtils.ts`: 151 lines.
- `apps/api/src/providers/renderer/smartEditComposer.ts`: 1474 lines.
- `apps/web/src/features/edit/SmartEditPanel.tsx`: 2972 lines.
- `apps/web/src/app/App.tsx`: 2529 lines.
...
1. Run full workspace verification before commit: `corepack pnpm typecheck`, `corepack pnpm lint`, `corepack pnpm test`, and `corepack pnpm build`.
2. Commit this memory render/scene utility extraction and documentation sync after `git diff --check` and `.agents/memory` tracking checks pass.
3. Push and deploy `codex/shopclip-optimization-cleanup` if the branch remains clean after commit.
4. Verify production after deploy with `/health`, `#project`, and `#studio`.
...
## 2026-06-09 Smart Edit Visual Filter Extraction Addendum

This pass is on `codex/shopclip-optimization-cleanup` and is backend-only. It avoids the frontend files the user is editing elsewhere.
...
- Kept ffmpeg command execution, URL/material materialization, subtitle overlay application, source-audio mixing, voiceover generation, upload publishing, and response mapping in `smartEditComposer.ts`.
- Preserved the existing public `smartEditOutputDimensions` export from `smartEditComposer.ts` by re-exporting it from the new module.
- Added focused coverage in `apps/api/src/providers/renderer/smartEditVisualFilters.test.ts`.

### Current File Sizes

- `apps/api/src/providers/renderer/smartEditComposer.ts`: 1085 lines, down from 1386 before this pass.
- `apps/api/src/providers/renderer/smartEditVisualFilters.ts`: 313 lines.
- `apps/api/src/providers/renderer/smartEditVisualFilters.test.ts`: 122 lines.
...
1. Commit this Smart Edit visual-filter extraction after `git diff --check` and `.agents/memory` tracking checks pass.
2. Continue backend cleanup only where helper ownership is clear, such as remaining `smartEditComposer.ts` audio/source materialization helpers or provider planning helpers.
3. Keep broad frontend refactors deferred until the user's separate frontend work is integrated.

## 2026-06-09 Smart Edit Source Audio Plan Extraction Addendum
...
- Moved pure helpers for:
  - global timeline duration calculation.
  - source-audio clip derivation from segments and timeline elements.
  - source-audio overlap detection.
  - safe file-token normalization.
- Kept source-audio materialization, silence insertion, concat list writing, ffmpeg lane mixing, and final track output generation in `smartEditComposer.ts`.
- Added focused coverage in `apps/api/src/providers/renderer/smartEditSourceAudioPlan.test.ts`.

### Current File Sizes

- `apps/api/src/providers/renderer/smartEditComposer.ts`: 965 lines, down from 1085 before this pass.
- `apps/api/src/providers/renderer/smartEditSourceAudioPlan.ts`: 144 lines.
- `apps/api/src/providers/renderer/smartEditSourceAudioPlan.test.ts`: 210 lines.
